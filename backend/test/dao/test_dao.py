...
class TestDAOCreateMethod:
    ...
    def dao_instance(self):
        # Define a JSON schema for validation during data insertion
        validator = {
            "$jsonSchema": {
                "bsonType": "object",
    ...
    # Test ID:1 - Valid data insertion
    def test_valid_data_insertion(self, dao_instance):
        ...
        assert '_id' in result, "Document should have an '_id' after successful insertion."

    # Test ID:2 - Insertion fails due to type mismatch
    def test_invalid_data_insertion(self, dao_instance):
        invalid_data = {"firstName": "John", "lastName": 12345, "email": "john.doe@example.com"}
        with pytest.raises(WriteError, match="type mismatch"):
            dao_instance.create(invalid_data)

    # Test ID:3 - Insertion fails due to duplicate entry
    ...
    # Test ID:4 - Insertion fails due to missing required fields
    def test_missing_required_fields(self, dao_instance):
        incomplete_data = {"firstName": "Eve"} 
        with pytest.raises(WriteError, match="missing fields"):
